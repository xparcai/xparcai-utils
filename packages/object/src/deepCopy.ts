...
export function deepCopy<T>(data: T, hash: any = new WeakMap()): T {
  ...
  if (isArray(data)) {
    for (const key in data)
      _data.push(deepCopy(data[key], hash))
    return _data
  }
  // 对象返回深拷贝的对象
  if (isObject(data)) {
    for (const key of Reflect.ownKeys(data as object)) {
      const dataKeyValue = (data as Record<string | symbol, any>)[key]
      if (typeof dataKeyValue === 'object' && dataKeyValue !== null) {
        _data[key] = deepCopy(dataKeyValue, hash)
      }
      else {
        _data[key] = dataKeyValue
      }
    }
    return _data
  }
